...
func TestGormUserRepo_Create(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "create duplicate email - should fail",
			input: &domain.User{
				TenantID:  1,
				FirstName: "Ana",
				LastName:  "López",
				Email:     "juan@test.com", // Same email
				Dni:       "87654321",
				Phone:     "1111111111",
				Gender:    "F",
				BirthDay:  time.Date(1990, 1, 1, 0, 0, 0, 0, time.UTC),
		...
		{
			name: "create duplicate phone - should fail",
			input: &domain.User{
				TenantID:  1,
				FirstName: "Luis",
				LastName:  "Martín",
...
func TestGormUserRepo_GetByEmail(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "get existing user by email - success",
			inputEmail: "juan@test.com",
			wantErr:    false,
			checkName:  true,
			expected:   "Juan",
		},
		{
			name:       "get by email - empty string - should fail",
			inputEmail: "",
			wantErr:    true,
		},
		{
			name:       "get by email - not found - should fail",
			inputEmail: "no@exists.com",
			wantErr:    true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := repo.GetByEmail(ctx, tt.inputEmail)
			if tt.wantErr && err == nil {
				t.Error("expected error but got nil")
			}
			if !tt.wantErr && err != nil {
...
func TestGormUserRepo_GetByPhone(t *testing.T) {
	db := setupTestDB(t)
	repo := NewGormUserRepository(db)
	ctx := contextWithTenant(context.Background(), 1)
	...
	tests := []struct {
		...
		{
			name:       "get existing user by phone - success",
		...
		{
			name:       "get by phone - empty string - should fail",
			inputPhone: "",
			wantErr:    true,
		},
		{
			name:       "get by phone - not found - should fail",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := repo.GetByPhone(ctx, tt.inputPhone)
			if tt.wantErr && err == nil {
				t.Error("expected error but got nil")
			}
			if !tt.wantErr && err != nil {
