...
func TestDepartmentService_Delete_WithPositionRepo(t *testing.T) {
	t.Run("delete department with associated positions - should fail", func(t *testing.T) {
		mockDeptRepo := NewMockDepartmentRepo()
		mockPosRepo := NewMockPositionRepo()
		dept := &domain.Department{ID: 1, Code: "ENG", Name: "Engineering"}
		mockDeptRepo.depts[1] = dept
		mockDeptRepo.codeIndex["ENG"] = dept
		mockDeptRepo.nameIndex["Engineering"] = dept

		pos := &domain.Position{ID: 1, DepartmentID: 1, Name: "Dev"}
		mockPosRepo.positions[1] = pos

		service := NewDepartmentService(mockDeptRepo, mockPosRepo)
		ctx := context.Background()
		err := service.Delete(ctx, 1)
		if err == nil {
			t.Error("expected error for department with positions but got nil")
		}
	})

	t.Run("delete department without positions - success", func(t *testing.T) {
		mockDeptRepo := NewMockDepartmentRepo()
		mockPosRepo := NewMockPositionRepo()
		dept := &domain.Department{ID: 1, Code: "ENG", Name: "Engineering"}
		mockDeptRepo.depts[1] = dept
