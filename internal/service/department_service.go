...
func (s *DepartmentService) Create(ctx context.Context, department *domain.Department) error {
	...
	if err := department.ValidateAll(); err != nil {
		return fmt.Errorf("validation error: %w", err)
	}

	existing, err := s.departmentRepo.GetByCode(ctx, department.Code)
	...
	existingName, err := s.departmentRepo.GetByName(ctx, department.Name)
	if err != nil && !errors.Is(err, domain.ErrDepartmentNotFound) {
